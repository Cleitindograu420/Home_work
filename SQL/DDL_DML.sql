...
SELECT pnome, unome FROM funcionario WHERE sexo = 'F';

SELECT DATE_PART('YEAR', AGE(CURRENT_DATE, dataNasc)) FROM funcionario WHERE LEFT (pnome, 1) = 'A' AND RIGHT(pnome, 1) = 'N';

SELECT MAX (salario) FROM funcionario WHERE numDept = 5;

SELECT MIN (salario) AS Menor_salario FROM funcionario, projeto WHERE projlocal = 'São Paulo';

SELECT nomeDependente FROM dependente
	JOIN funcionario ON dependente.fcpf = funcionario.cpf
	WHERE LEFT (pnome, 1) = 'A';

SELECT fcpf FROM dependente WHERE nomeDependente = 'Joana Bagual';
...
WHERE
    f.numDept = d.dnumero
    AND f.cpf = t.fcpf
    AND t.projnumero = p.projNumero
    AND p.projNome = 'Cama Hiperbarica';

SELECT cpf, cpfSuperv FROM funcionario;
-- LETRA J
SELECT MAX(horas) FROM trabalhaEm

SELECT pnome FROM funcionario WHERE cpfSuperv IS NULL

SELECT pnome FROM funcionario
	JOIN trabalhaEm ON funcionario.cpf = trabalhaEm.fcpf
	WHERE trabalhaEm.horas < 30 AND trabalhaEm.horas >= 15